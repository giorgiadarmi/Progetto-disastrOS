...
void internal_msgQueueRead() {
    ...
    if (!msgSubqueue) {
        printf("[ERRORE] Non è in grado di leggere msg: la coda di messaggi (fd =% d) è piena!\n", mqdes);
        running->syscall_retvalue = DSOS_EMQ_READ;
        return;
    }
    ...
    if (msg->msg_len > msg_len) {
        printf("[ERRORE] La lunghezza del messaggio %d deve essere maggiore %d!\n", msg_len, msg->msg_len);
        running->syscall_retvalue = DSOS_EMQ_READ;
        return;
    }
